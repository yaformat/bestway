<template>
  <div>
    <VCardText class="d-flex justify-space-between flex-wrap gap-4">
      <VRow>
        ...
        <VCol cols="12" sm="8" class="text-end text-right">
          <VBtn @click="isCreateDialogVisible = true" class="ml-2">Создать Перемещение</VBtn>
        </VCol>
      </VRow>
    </VCardText>
    <Filters
    ...
    >
      ...
      <!-- Stock To -->
      <template #item.stock_to="{ item }">
        <div>
          <div v-if="item.stock_to"><span class="text-sm">{{ item.stock_to.name }}</span></div>
        </div>
      </template>
      <!-- User -->
      ...
      <template #item.actions="{ item }">
        <div class="text-no-wrap">
          <VBtn 
            :to="{ name: 'stock-transfer-id', params: { id: item.id } }" 
            size="small" 
            variant="outlined"
          >
    ...
    >
      <StockTransferCreateForm :model="newEntity" />
    </EntityDialog>
  </div>
</template>
...
const itemsStore = useStockTransferStore()
...
const {
  ...
  updateEntity,
  openEditDialog
} = useEntityActions(itemsStore, 'перемещения', getItems)
