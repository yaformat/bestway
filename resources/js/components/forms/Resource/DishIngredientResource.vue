...
const initializeLosses = () => {
  ...
  if (newActiveLosses.length > 0) {
    ...
    emit('update-resource-losses', props.id, appliedLosses)
  }
}
...
const updateBrutto = (e) => {
  emit('update-resource-weight', props.id, 'weight_brutto', e.target.value)
}

const updateNetto = (e) => {
  emit('update-resource-weight', props.id, 'weight_netto', e.target.value)
}

const updateOutput = (e) => {
  emit('update-resource-weight', props.id, 'weight_output', e.target.value)
}
</script>
