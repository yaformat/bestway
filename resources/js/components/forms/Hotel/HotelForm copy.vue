...
import { useRestTypeStore } from '@/stores/restTypeStore';
...
const restTypeStore = useRestTypeStore();
...
const loadHotel = async () => {
  try {
    const hotel = await hotelStore.getItem(route.params.id);
...
const loadReferenceData = async () => {
  try {
    ...
    await directionStore.fetchAll();
    directions.value = directionStore.items;
    
    // Загрузка курортов
    await resortStore.fetchAll();
    resorts.value = resortStore.items;
    
    // Загрузка видов отдыха
    await restTypeStore.fetchAll();
    restTypes.value = restTypeStore.items;
  } catch (error) {
...
const save = async () => {
  ...
  try {
    ...
    showNotification('Ошибка сохранения отеля', 'error');
  } finally {
    saving.value = false;
  }
};
