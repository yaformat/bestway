...
export const isNavLinkActive = (link, router) => {
  ...
  // return matchedRoutes.some(route => {
  //   return route.name === resolveRoutedName || route.meta.navActiveLink === resolveRoutedName || router.currentRoute.value.name.startsWith(link.to.name)
  // })

  return matchedRoutes.some(route => {
    return route.meta.navActiveLink === resolveRoutedName || router.currentRoute.value.path.startsWith(link.to.path)
  })
}
