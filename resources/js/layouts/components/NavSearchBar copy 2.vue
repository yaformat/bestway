  ...
  nodes: [
    {
      ...
      port: '8190',      // For Typesense Cloud use 443
...
// Custom display names for collections
const collectionDisplayNames = {
  ingredient_resources: 'Ingredients',
  household_resources: 'Household Items',
  tech_cards: 'Tech Cards'
}

// Fetch the list of collections and perform multi-search
const performSearch = async () => {
  try {
    ...
    const searches = collections.map(collection => ({
      ...
      q: searchQuery.value,
      query_by: 'name' // Adjust this based on your schema
    }))

    const response = await typesense.multiSearch.perform({ searches })
    searchResult.value = response.results.reduce((acc, result) => {
      const collectionName = result.collection
      const displayName = collectionDisplayNames[collectionName] || collectionName
      if (!acc[displayName]) {
        acc[displayName] = []
      }
      acc[displayName].push(...result.hits.map(hit => {
        const document = hit.document
        return {
          ...document,
          photo: document.photo ? document.photo.url : null
        }
      }))
      return acc
    }, {})
        ...
        <template v-for="(group, groupName) in searchResult" :key="groupName">
          ...
          <VList v-show="searchQuery.length && !!group.length" density="compact" class="app-bar-search-list">
            <VListItem v-for="item in group" :key="item.id" link @click="redirectToSuggestedOrSearchedPage(item)">
              <template #prepend>
                <VImg v-if="item.photo" :src="item.photo" class="me-3" :width="50" />
              </template>
              <template #append>
